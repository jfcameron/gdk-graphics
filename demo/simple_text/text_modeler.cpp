...
text_modeler::text_modeler(gdk::graphics::context_ptr_type pContext, gdk::graphics::material_ptr_type aMaterial) 
: m_BatchModeler(pContext, aMaterial ? aMaterial : [&]() {
    auto pTexture = pContext->make_texture(gdk::graphics::ext::make_from_png(GLYPH_PNG_DATA).first);
    auto pMaterial(pContext->make_material(pContext->make_alpha_cutoff_shader(), material::render_mode::opaque));
...
void text_modeler::set_text(const std::string &string) {
    [[maybe_unused]] static const auto GLYPH_PER_ROW_OR_COLUMN(8);
    static const auto GLYPH_UV_SIZE(1/8.f);
    static const std::unordered_map<char, graphics::vector2_type> CHAR_TO_UV({
...
const gdk::graphics::const_model_ptr_type text_modeler::model() const { return m_BatchModeler.model(); }
const gdk::graphics::const_material_ptr_type text_modeler::material() const { return m_BatchModeler.material(); }
