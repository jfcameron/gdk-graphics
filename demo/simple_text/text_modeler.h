...
/// TODO: probably move this specifically to a demo. i should provide at least one text renderer in ext though
///
class text_modeler final {
public:
    //! get the model, contains the uploaded model data for the batch
    const gdk::graphics::const_model_ptr_type model() const;

    //! get the material, used to render the model
    const gdk::graphics::const_material_ptr_type material() const;
    ...
    text_modeler(gdk::graphics::context_ptr_type pContext, gdk::graphics::material_ptr_type aMaterial = {});

private:
    gdk::graphics::batch_modeler m_BatchModeler;
};
