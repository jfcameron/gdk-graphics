...
namespace gdk::graphics {
    ...
    public:
        ...
        std::optional<active_attribute_info> tryGetActiveAttribute(
            const std::string &aAttributeName) const;
        ...
        bool try_set_uniform(const std::string &aName, const vector2_type &aValue) const;
        //! assign a float3 uniform from a 3 component vector
        bool try_set_uniform(const std::string &aName, const vector3_type &aValue) const;
        //! assign a float4 uniform from a 4 component vector
        bool try_set_uniform(const std::string &aName, const vector4_type &aValue) const;
        ...
        bool try_set_uniform(const std::string &aName, const std::vector<vector2_type> &avalue) const; 
        //! assign a value to a float3 uniform array from a vector of float3s
        bool try_set_uniform(const std::string &aName, const std::vector<vector3_type> &avalue) const;
        //! assign an RGBA colour to a float4 uniform
        bool try_set_uniform(const std::string &aName, const color &aValue) const;

        //! assign a value to a float4 uniform array from a vector of float4s
        bool try_set_uniform(const std::string &aName, const std::vector<vector4_type> &avalue) const; 
        ...
        //bool try_set_uniform(const std::string &aName, const bool &aValue) const;
        //! assign a value to a bvec2 uniform
        //bool try_set_uniform(const std::string &aName, const boolean2_uniform_type &a) const;
        //! assign a value to a bvec3 uniform
        //bool try_set_uniform(const std::string &aName, const boolean3_uniform_type &a) const;
        //! assign a value to a bvec4 uniform
        //bool try_set_uniform(const std::string &aName, const boolean4_uniform_type &a) const;

        //! assign a value to a bool uniform array 
        //bool try_set_uniform(const std::string &aName, const std::vector<bool> &aValue) const;
        ...
        bool try_set_uniform(const std::string &aName, const mat2x2_type &avalue) const; 
        //! assign a mat2x2 uniform array from a vector of mat2x2s
        bool try_set_uniform(const std::string &aName, const std::vector<mat2x2_type> &avalue) const;*/

        //! assign a mat3x3 uniform from a mat3x3
        bool try_set_uniform(const std::string &aName, const mat3x3_type &avalue) const;
        ...
        bool try_set_uniform(const std::string &aName, const matrix4x4_type &avalue) const; 
        //! assign a mat4x4 uniform array from a vector of mat4x4s
        bool try_set_uniform(const std::string &aName, const std::vector<matrix4x4_type> &avalue) const; 

        //TODO: texture needs to support more than tex2d!
        //! bind a texture to the context then assign it to a texture uniform
        bool try_set_uniform(const std::string &aName, const gdk::graphics::webgl1es2_texture &aTexture,
        ...
        /// `_Model`, `_View`, `_Projection` and `_MVP`, without the four string hashes -- the
        /// locations are resolved once, when the program links.
        ...
        //bool try_set_uniform(const std::string &aName, const gdk::graphics::texture &aTexture) const;
        ...
        ///
        /// **Built fresh, and the caller owns it.** These used to be lazily instantiated statics shared
        /// by everything in the process, which is a gl *object* held outside any gl context -- fine
        /// while there was only ever one, and wrong the moment a second window appeared, because a
        /// handle made in one context means nothing in another.
        /// \see ROADMAP_GDK_GRAPHICS.md, and ROADMAP_GDK_WINDOW.md §6g for how this was found
        ...
        ///
        /// Suitable for text, gui elements and 2d sprites; extremely lightweight. \see
        /// make_pink_shader_of_death for why this is a factory rather than a shared instance.
    ...
    private:
        ...
        ///
        /// Their names are fixed by this library's shader convention and their locations are fixed the
        /// moment the program links, so finding them by string hash per entity per frame was pure
        /// repeated work -- measured at 19% of submission cost, 8.8 ms of a 66 ms frame at 8000
        /// entities. \see ROADMAP_GDK_GRAPHICS.md
        ///
        /// -1 is gl's "no such uniform", and is also what glUniform* silently ignores, so a shader
        /// that does not declare one of these needs no special case.
